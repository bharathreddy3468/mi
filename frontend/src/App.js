...
// Original Mock Interview Components (keeping existing functionality)
...
const DynamicVideoInterview = ({ sessionId, setupId, onInterviewComplete }) => {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (audioRef.current) {
          ...
          // Start playing - animation text will change when audio actually plays
      ...
      reader.onloadend = async () => {
        ...
        try {
          ...
          await axios.post(`${API}/interview/${sessionId}/answer`, {
            ...
            question_text: currentQuestion.question_text,  // Pass the actual question text
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-2 md:py-4 px-2 md:px-4">
      <div className="max-w-7xl mx-auto h-[calc(100vh-2rem)] md:h-[calc(100vh-4rem)]">
        <Card className="shadow-2xl border-0 overflow-hidden h-full flex flex-col">
          <CardHeader className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-4 py-3 flex-shrink-0">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
              <div>
                <CardTitle className="text-lg md:text-xl">AI Mock Interview</CardTitle>
                {currentQuestion && (
              ...
              {isRecording && (
                <div className="flex items-center space-x-2 bg-red-500 px-3 py-1.5 rounded-full">
                  <div className="h-2 w-2 bg-white rounded-full animate-pulse"></div>
                  <span className="text-white font-semibold text-xs md:text-sm">Recording</span>
                </div>
              )}
            </div>
          </CardHeader>

          <CardContent className="p-3 md:p-4 flex-1 overflow-hidden">
            {/* Responsive Grid: Better proportions to show both columns */}
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-3 md:gap-4 h-full">
              
              {/* Left Column: Video Preview - Takes 3/5 on desktop */}
              <div className="lg:col-span-3 flex flex-col h-full min-h-0">
                <div className="relative bg-gray-900 rounded-lg overflow-hidden flex-1">
                  ...
                  {/* TTS Playing Animation - Shows when AI is actually speaking */}
                      ...
                      <div className="text-white text-center space-y-4 px-4">
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-center space-x-2">
                            ...
                            <p className="text-lg md:text-xl font-bold">AI is Speaking</p>
                          </div>
              ...
              {/* Right Column: Question, Timer, and Controls - Takes 2/5 on desktop */}
              <div className="lg:col-span-2 flex flex-col gap-3 h-full overflow-y-auto min-h-0">
                
                {/* Question Display - Readable size */}
                {currentQuestion && !isPlayingAudio && !isProcessing && (
                  <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200 shadow-md animate-fadeIn flex-shrink-0">
                    <CardContent className="pt-4 pb-4 px-4">
                      <div className="flex items-start space-x-2">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm md:text-base lg:text-lg text-gray-900 font-medium leading-relaxed">
                ...
                {/* Timer - Readable */}
                {isRecording && (
                  <Card className="bg-white shadow-md flex-shrink-0">
                    <CardContent className="py-3 px-4">
                      <div className="flex items-center justify-center space-x-2">
                        <Clock className="h-5 w-5 text-gray-600" />
                        <span className="text-2xl md:text-3xl font-bold text-gray-900">{formatTime(timeLeft)}</span>
                      </div>
                      <p className="text-center text-xs text-gray-500 mt-1">Time left</p>
                    </CardContent>
                  </Card>
                )}
                ...
                <div className="flex flex-col gap-3 mt-auto flex-shrink-0">
                  {!interviewStarted ? (
                    <Button
                      ...
                      size="lg"
                      className="bg-green-600 hover:bg-green-700 text-white py-5 md:py-6 text-base md:text-lg shadow-lg hover:shadow-xl transition-all"
                    ...
                    <>
                      <Button
                        ...
                        className="bg-blue-600 hover:bg-blue-700 text-white py-5 md:py-6 text-base md:text-lg shadow-lg hover:shadow-xl transition-all disabled:opacity-50"
                      >
                        {isProcessing ? (
                          <div className="flex items-center space-x-2">
                      ...
                      <Button
                        ...
                        variant="destructive"
                        disabled={!isRecording || isPlayingAudio || isLoadingQuestion || isProcessing}
                        className="bg-red-600 hover:bg-red-700 text-white py-3 md:py-4 text-sm md:text-base shadow-lg hover:shadow-xl transition-all disabled:opacity-50"
                      >
                        End Interview Now
                      </Button>
                    </>
...
// Mock Interview Page Component (existing functionality)
